...
static void esp_zb_buttons_handler(switch_func_pair_t *button_func_pair)
{
    if (button_func_pair->func == SWITCH_ONOFF_TOGGLE_CONTROL) {
        ...
        cmd_req.zcl_basic_cmd.src_endpoint = COORDINATOR_SWITCH_ENDPOINT;
...
static void send_toggle_command(void *pvParameters) {
    ...
    cmd_req.address_mode = ESP_ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
    cmd_req.on_off_cmd_id = ESP_ZB_ZCL_CMD_ON_OFF_TOGGLE_ID;
    ESP_LOGI(TAG, "Sending automatic 'on_off toggle' command");
    esp_zb_zcl_on_off_cmd_req(&cmd_req);
...
static void user_find_cb(esp_zb_zdp_status_t zdo_status, uint16_t addr, uint8_t endpoint, void *user_ctx)
{
    if (zdo_status == ESP_ZB_ZDP_STATUS_SUCCESS) {
        ...
        bind_req.src_endp = COORDINATOR_SWITCH_ENDPOINT;
...
static void esp_zb_task(void *pvParameters)
{
    ...
    esp_zb_ep_list_t *esp_zb_on_off_switch_ep = esp_zb_on_off_switch_ep_create(COORDINATOR_SWITCH_ENDPOINT, &switch_cfg);
    esp_zb_device_register(esp_zb_on_off_switch_ep);
    esp_zb_set_primary_network_channel_set(ZIGBEE_CHANNEL);
