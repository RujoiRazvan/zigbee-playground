...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct)
{
    ...
    case ESP_ZB_BDB_SIGNAL_DEVICE_REBOOT:
        if (err_status == ESP_OK) {
            ...
            esp_zb_bdb_start_top_level_commissioning(ESP_ZB_BDB_MODE_NETWORK_STEERING); // try to steering, when initialization failure
        }
        break;
...
static esp_err_t zb_attribute_handler(const esp_zb_zcl_set_attr_value_message_t *message)
{
    ...
    read_button_state(); // Check reset button

    ESP_RETURN_ON_FALSE(message, ESP_FAIL, TAG, "Empty message");
    ESP_RETURN_ON_FALSE(message->info.status == ESP_ZB_ZCL_STATUS_SUCCESS, ESP_ERR_INVALID_ARG, TAG, "Received message: error status(%d)",
                        message->info.status);

    ESP_LOGI(TAG, "---- Zigbee ZCL Attribute Write Received ----");
    ESP_LOGI(TAG, "  Endpoint: %d", message->info.dst_endpoint);
    ESP_LOGI(TAG, "  Cluster:  0x%04X", message->info.cluster);
    ESP_LOGI(TAG, "  Attr ID:  0x%04X", message->attribute.id);
    ESP_LOGI(TAG, "  Data type: 0x%02X", message->attribute.data.type);
    ESP_LOGI(TAG, "  Data size: %d", message->attribute.data.size);

    if (message->attribute.data.value) {
        // Print hex values of data
        char hex_buf[64] = {0};
        int max_print = message->attribute.data.size < 32 ? message->attribute.data.size : 32;
        for (int i = 0; i < max_print; i++) {
            sprintf(&hex_buf[i * 3], "%02X ", ((uint8_t *)message->attribute.data.value)[i]);
        }
        ESP_LOGI(TAG, "  Data (hex): %s", hex_buf);
    }
    ...
    if (message->info.dst_endpoint == HA_ESP_LIGHT_ENDPOINT) {
        if (message->info.cluster == ESP_ZB_ZCL_CLUSTER_ID_ON_OFF) {
            if (message->attribute.id == ESP_ZB_ZCL_ATTR_ON_OFF_ON_OFF_ID && message->attribute.data.type == ESP_ZB_ZCL_ATTR_TYPE_BOOL) {
                ...
                ESP_LOGI(TAG, "Action: Light sets to %s", light_state ? "On" : "Off");
